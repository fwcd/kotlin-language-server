...
class SourcePath(
    ...
    private inner class SourceFile(
        ...
        val extension: String =
            uri.fileExtension
                ?: "kt" // TODO: Use language?.associatedFileType?.defaultExtension again
        val isScript: Boolean = extension == "kts"

        val kind: CompilationType =
            if (path?.fileName?.toString()?.endsWith(".gradle.kts") == true)
                CompilationType.BUILD_SCRIPT
        ...
        private fun allIncludingThis(): Collection<KtFile> =
            parseIfChanged().let {
                if (isTemporary) (all().asSequence() + sequenceOf(parsed!!)).toList() else all()
            }
    ...
    fun latestCompiledVersion(uri: URI): CompiledFile = sourceFile(uri).prepareCompiledFile()
    ...
    fun compileFiles(all: Collection<URI>): BindingContext {
        ...
        val (changedBuildScripts, changedSources) =
            allChanged.partition { it.kind == CompilationType.BUILD_SCRIPT }
        ...
        val buildScriptsContext =
            compileAndUpdate(changedBuildScripts, CompilationType.BUILD_SCRIPT)
        ...
        val combined =
            listOfNotNull(buildScriptsContext, sourcesContext) + same.map { it.compiledContext!! }

        return CompositeBindingContext.create(combined)
    }

    fun compileAllFiles() {
        // TODO: Investigate the possibility of compiling all files at once, instead of iterating
        // here
        // At the moment, compiling all files at once sometimes leads to an internal error from the
        // TopDownAnalyzer
        files.keys.forEach {
    ...
    /**
     * Saves a file. This generates code for the file and deletes previously generated code for this
     * file.
     */
    fun save(uri: URI) {
        files[uri]?.let {
            if (!it.isScript) {
                // If the code generation fails for some reason, we generate code for the other
                // files anyway
                try {
                    cp.compiler.removeGeneratedCode(listOfNotNull(it.lastSavedFile))
                    it.module?.let { module ->
                        it.compiledContext?.let { context ->
                            cp.compiler.generateCode(
                                module,
                                context,
                                listOfNotNull(it.compiledFile),
                            )
    ...
    /**
     * Refreshes the indexes. If already done, refreshes only the declarations in the files that
     * were changed.
     */
    private fun refreshWorkspaceIndexes(oldFiles: List<SourceFile>, newFiles: List<SourceFile>) =
        asyncExecutor.execute {
            if (indexEnabled) {
                ...
                // Index the new declarations in the Kotlin source files that were just compiled,
     ...
     * were changed.
     */
    private fun refreshDependencyIndexes(module: ModuleDescriptor) =
        asyncExecutor.execute {
            if (indexEnabled) {
        ...
        files
            .flatMap { file ->
                ...
                if (compiledFile != null && module != null) {
                    module
                        .getPackage(compiledFile.packageFqName)
                        .memberScope
                        .getContributedDescriptors(DescriptorKindFilter.ALL) { name ->
                            compiledFile.declarations.map { it.name }.contains(name.toString())
                        }
                } else {
                    listOf()
                }
