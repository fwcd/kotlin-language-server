...
fun resolveMain(file: CompiledFile): Map<String, Any> {
    ...
    findCompanionObjectMain(parsedFile)?.let { companionMain ->
        return mapOf(
            "mainClass" to (companionMain.first ?: ""), "range" to range(file.content, companionMain.second)
        )
    }
...
private fun findCompanionObjectMain(file: KtFile): Pair<String?, TextRange>? =
    file.declarations.flatMap { topLevelDeclaration ->
            ...
            companionObjectInternal.takeIf {
                companionObjectInternal is KtNamedFunction && "main" == companionObjectInternal.name && companionObjectInternal.text.startsWith(
                    "@JvmStatic"
                )
            }
        }?.let {
