...
fun findReferences(file: Path, offset: Int, sp: SourcePath): List<Location> {
    return doFindReferences(file, offset, sp)
            .map { location(it) }
            .toList()
}

private fun doFindReferences(file: Path, offset: Int, sp: SourcePath): Collection<KtElement> {
