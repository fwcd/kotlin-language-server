...
class SignatureHelpTest: LanguageServerTestFixture("signatureHelp") {
    ...
    @Ignore @Test fun `find active parameter`() {
        val help = languageServer.textDocumentService.signatureHelp(position(file, 5, 32)).get()!!

        assertThat(help.activeParameter, equalTo(1))
    }

    @Test fun `find active parameter in the middle of list`() {
        val help = languageServer.textDocumentService.signatureHelp(position(file, 5, 30)).get()!!
    ...
    @Test fun `find active signature using number of arguments`() {
        val help = languageServer.textDocumentService.signatureHelp(position(file, 6, 34)).get()!!
